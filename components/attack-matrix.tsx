      ...
      >
        ...
        {ATTACKER_TEAMS.map((attacker) =>
          ATTACKER_TEAMS.map((target) => {
            ...
            return (
              <div key={key} className="attack-matrix-cell-wrap">
                <button
                  type="button"
                  className={`attack-matrix-cell ${target.border} ${count > 0 ? "has-attacks" : "empty"} ${isExpanded ? "expanded" : ""}`}
                  aria-label={
                    count > 0
                      ? `${count} attack${count === 1 ? "" : "s"} from ${attacker.name} to ${target.name}`
                      : `No attacks from ${attacker.name} to ${target.name}`
                  }
                ...
                {isExpanded && cellAttackList.length > 0 ? (
                  <div className="attack-matrix-expanded" role="region" aria-label={`Attacks from ${attacker.name} to ${target.name}`}>
