...
export function LiveAiDegraded({ onReturnVerified }: { onReturnVerified: () => void }) {
  ...
  return <section className={styles.page} aria-labelledby="degraded-title" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setPaused(false); }}>
